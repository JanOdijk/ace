...
plusdotdot = re.compile(r'\+\.\.') # is actually wrong
plusdotdotdot = re.compile(r'\+\.\.\.')
ltstr = r'\[<\]'
...
def test():
    testpairs = [('<oppe tap> [:op de trap] [=op de trap]', 'op de trap'),
                 ('< oppe tap > [:op de trap] [=op de trap]', 'op de trap'),
                 ('<oppe tap>  [=op de trap] [:op de trap]', 'op de trap'),
                 ('<oppe tap> [: op de trap] [= op de trap]', 'op de trap'),
                 ('oppe [: op de] tap [: trap] [= op de trap]', 'op de trap'),
                 ('<oppe [: op de] tap [: trap]> [= op de trap]', 'op de trap'),
                 ('die kan niet +...', 'die kan niet'),
                 ('kleine [=! singing] +...', 'kleine'),
                 ('=++ klok +...', 'klok'),
                 ('bij de opleiding was ik 50 of zo toen ik de op op opleiding heb gedaan voor uh voor uh hui BEROEP2 [*s:sempar]',
                  'bij de opleiding was ik 50 of zo toen ik de op op opleiding heb gedaan voor uh voor uh hui BEROEP2'),
                 ('kan nu niet , nu niet aanklee [: aankleden] e [: ] .  ', 'kan nu niet , nu niet aankleden .  ')
                ]

    for (instr, rawgoldstr) in testpairs:
        rawoutstr = cleantext(instr, repkeep=True)
        outstr =  ' '.join(rawoutstr.split())
        goldstr = ' '.join(rawgoldstr.split())
        if  outstr == goldstr:
            print ('OK:', instr + ':', outstr, '==', goldstr)
        else:
            print ('NO:', instr + ':', outstr, '!=', goldstr)
